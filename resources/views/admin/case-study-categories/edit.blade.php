...
@section('title', 'Edit Case Study Category')
...
@section('content')
    ...
    <form action="{{ route('admin.case-study-categories.update', $caseStudyCategory) }}" method="POST" class="max-w-2xl space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="inline-flex items-center gap-1.5 rounded-lg bg-fuchsia-600 px-5 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-fuchsia-700">Update Category</button>
        </div>
    </form>
@endsection
