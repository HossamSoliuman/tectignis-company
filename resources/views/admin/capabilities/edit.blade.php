...
@section('content')
    <div class="mb-4">
        <a href="{{ route('admin.capabilities.index') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-fuchsia-600 hover:underline"><x-admin.icon name="arrow-left" class="h-4 w-4" /> Back to Capabilities</a>
    </div>

    <form action="{{ route('admin.capabilities.update', $capability) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 xl:grid-cols-[1fr_18rem] gap-6 items-start">
            ...
            <div class="space-y-4 xl:sticky xl:top-20 self-start">
                <div class="rounded-xl border border-slate-200 bg-white p-5 space-y-5">
                    ...
                    <x-admin.image-field name="icon" label="Icon Image" :current="$capability->icon" />
                    <x-admin.image-field name="banner_image" label="Banner Image" :current="$capability->banner_image" />
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Sort Order</label>
                        <input type="number" name="sort_order" value="{{ old('sort_order', $capability->sort_order) }}" min="0"
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="checkbox" id="is_active" name="is_active" value="1"
                            {{ old('is_active', $capability->is_active) ? 'checked' : '' }}
