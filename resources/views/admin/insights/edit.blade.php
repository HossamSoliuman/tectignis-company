...
@section('content')
    <div class="mb-4">
        <a href="{{ route('admin.insights.index') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-fuchsia-600 hover:underline"><x-admin.icon name="arrow-left" class="h-4 w-4" /> Back to Insights</a>
    </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-[1fr_18rem] gap-6 items-start">
            ...
            <div class="space-y-4 xl:sticky xl:top-20 self-start">
                <div class="rounded-xl border border-slate-200 bg-white p-5 space-y-5">
                    ...
                    <x-admin.image-field name="image" label="Featured Image" :current="$insight->image" />
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Sort Order</label>
                        <input type="number" name="sort_order" value="{{ old('sort_order', $insight->sort_order) }}" min="0"
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="checkbox" id="is_published" name="is_published" value="1"
                            {{ old('is_published', $insight->is_published) ? 'checked' : '' }}
